...
impl<T> FastVec<T> {
    ...
    pub fn push(&mut self, t: T) {
        ...
        let old_ptr = self.ptr_to_data;

        let data = MALLOC.malloc(size_of::<T>() * new_capacity) as *mut T;

        for i in 0..self.len {
            unsafe {
                let val = ptr::read(old_ptr.add(i));
                ptr::write(data.add(i), val);
            }
        }

        unsafe {
            ptr::write(data.add(self.len), t);
        }

        MALLOC.free(old_ptr as *mut u8);

        self.ptr_to_data = data;
        self.len += 1;
    ...
    pub fn remove(&mut self, i: usize) {
        ...
        unsafe {
            let _removed = std::ptr::read(self.ptr_to_data.add(i));
            for j in i + 1..self.len {
                let val = std::ptr::read(self.ptr_to_data.add(j));
                std::ptr::write(self.ptr_to_data.add(j - 1), val);
            }
            self.len -= 1;
        }
    }
    ...
    pub fn clear(&mut self) {
        for i in 0..self.len {
            unsafe {
                let _to_drop = ptr::read(self.ptr_to_data.add(i));
            }
        }
        MALLOC.free(self.ptr_to_data as *mut u8);
        self.ptr_to_data = null_mut();
